...
docker run --rm -ti --name structured-streaming \
--network=streaming-network \
-v ./:/spark \
-v spark_data:/data \
...
unigap/spark:3.5 bash -c "(cd /spark/10-structured-streaming && zip -r /tmp/util.zip util/*) &&
...
--py-files /tmp/util.zip,/spark/10-structured-streaming/spark.conf \
--deploy-mode client \
--master yarn \
